...
std::mutex work_mutex[NUM_THREADS];
...
void thread_helper(int threadID, int targetThread) {
    ...
    if(!q[targetThread].empty()){
        ...
        //If the thread is stealing task it pops from the end of the target queue, otherwise from the front of its own queue
...
void par_rec_mm_2(float *z, const float *x, const float *y, const int size,
    ...
    add_task_to_thread_queue(TaskInfo(z11, x12, y21, size / 2, c_sync), threadID);
    add_task_to_thread_queue(TaskInfo(z12, x12, y22, size / 2, c_sync), threadID);
    add_task_to_thread_queue(TaskInfo(z21, x22, y21, size / 2, c_sync), threadID);
...
void par_rec_mm(float *z, const float *x, const float *y, const int size,
    ...
    if (size <= THRESHOLD) {
        ...
        add_task_to_thread_queue(TaskInfo(z11, x11, y11, size / 2, local_sync), threadID);
        add_task_to_thread_queue(TaskInfo(z12, x11, y12, size / 2, local_sync), threadID);
        add_task_to_thread_queue(TaskInfo(z21, x21, y11, size / 2, local_sync), threadID);
...
void worker(int threadID){
    ...
    while(!empty){
        if(q[threadID].size() != 0){
            ...
            while(!empty && q[randID].size() == 0){
                randID = targetThreadSelect(STEAL);
            }
            thread_helper(threadID, randID);
            randID=threadID;
        }
    }
}
